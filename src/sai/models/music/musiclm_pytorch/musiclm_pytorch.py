...
from sai.models.music.audiolm_pytorch import AudioLM
from sai.models.music.audiolm_pytorch.utils import AudioConditionerBase
...
class AudioSpectrogramTransformer(nn.Module):
    ...
    def forward(self, x, force_no_patch_dropout=False, return_all_layers=False):
        ...
        rounded_height, rounded_width = map(
            lambda args: round_down_nearest_multiple(*args), ((height, patch_height), (width, patch_width))
        )

        if (height, width) != (
            rounded_height,
            rounded_width,
        ):  # just keep printing to be annoying until it is fixed
            print_once(
...
class TextTransformer(nn.Module):
    ...
    @beartype
    def forward(self, x=None, raw_texts: Optional[List[str]] = None, mask=None, return_all_layers=False):
...
class MultiLayerContrastiveLoss(nn.Module):
        ...
        *,
        ...
        klass = (
            ...
            if sigmoid_contrastive_loss
                ...
                SoftmaxContrastiveLearning, decoupled_contrastive_learning=decoupled_contrastive_learning
            )
        )
    ...
    def forward(self, *, audio_layers, text_layers):
        ...
        audio_latents = (
            einsum("l b d, l d e -> l b e", audio_embeds, self.audio_latent_weight) + self.audio_latent_bias
        )
        ...
        text_latents = (
            einsum("l b d, l d e -> l b e", text_embeds, self.text_latent_weight) + self.text_latent_bias
        )
...
class MuLaN(nn.Module):
    ...
    def __init__(
        ...
        if hierarchical_contrastive_loss:
            num_layers = default(
                hierarchical_contrastive_loss_layers, min(audio_transformer.depth, text_transformer.depth) - 1
            )
            assert num_layers > 0

            self.register_buffer(
                "text_layers_indices", interspersed_indices(num_layers, text_transformer.depth)
            )
            self.register_buffer(
                "audio_layers_indices", interspersed_indices(num_layers, audio_transformer.depth)
            )
    ...
    def forward(
        ...
        hierarchical_cl_loss = self.multi_layer_contrastive_learning(
            audio_layers=audio_layers, text_layers=text_layers
        )
...
class MusicLM(nn.Module):
    ...
    def __init__(self, audio_lm: AudioLM, mulan_embed_quantizer: MuLaNEmbedQuantizer):
        super().__init__()
        assert not exists(
            audio_lm.audio_conditioner
        ), "mulan must not have been passed into AudioLM. it will be managed externally now, embedding the text into the joint embedding space for text-to-audio synthesis"
    ...
    def forward(self, text: str, num_samples=1, **audio_lm_kwargs):
        ...
        sims = torch.cat(
            [mulan(texts=texts, wavs=music, return_similarities=True) for music in samples], dim=0
        )

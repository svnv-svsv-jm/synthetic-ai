...
@beartype
def get_embeds(
    embeddings: nn.Embedding, codes: torch.Tensor, pad_id=-1, return_mask=False, mask_pad_pos_to=0
):
...
class SemanticTransformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        audio_text_condition: bool = False,
        cond_as_self_attn_prefix: bool = False,
    ...
    def forward_with_cond_scale(
        ...
        **kwargs,
        ...
        null_logits, null_new_kv_cache = self.forward(
            *args, cond_drop_prob=1.0, kv_cache=next(kv_cache, None), return_kv_cache=True, **kwargs
        )
    ...
    def forward(
        ...
        *,
        ...
        tokens, kv_cache = self.transformer(
            tokens,
            context=text_embeds,
...
class CoarseTransformer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        cond_drop_prob: float = 0.5,
        grad_shrink_alpha: float = 0.1,
        project_semantic_logits: bool = True,
        rel_pos_bias: bool = True,
        flash_attn: bool = False,
        **kwargs: ty.Any,
    ):
        ...
        self.coarse_logit_weights = nn.Parameter(
            torch.randn(num_coarse_quantizers, codebook_size_with_eos, dim)
        )
    ...
    def forward_with_cond_scale(
        self, *args, cond_scale=3, return_kv_cache=False, kv_cache=None, embed_cache=None, **kwargs
    ):
        ...
        (semantic_logits, coarse_logits), (new_kv_cache, new_embed_cache) = self.forward(
            *args,
            cond_drop_prob=0.0,
            return_cache=True,
            kv_cache=next(iter_kv_cache, None),
            embed_cache=next(iter_embed_cache, None),
            **kwargs,
        )
        new_kv_caches.append(new_kv_cache)
        ...
        if cond_scale == 1 or not self.has_condition:
            ...
            return (semantic_logits, coarse_logits), (
                torch.stack(new_kv_caches),
                torch.stack(new_embed_caches),
            )

        (null_semantic_logits, null_coarse_logits), (null_new_kv_cache, null_new_embed_cache) = self.forward(
            *args,
            cond_drop_prob=1.0,
            return_cache=True,
            kv_cache=next(iter_kv_cache, None),
        ...
        if exists(null_semantic_logits):
            scaled_semantic_logits = (
                null_semantic_logits + (semantic_logits - null_semantic_logits) * cond_scale
            )
    ...
    def forward(
        ...
        *,
        ...
        coarse_token_ids, semantic_token_ids = map(
            lambda t: rearrange(t, "b ... -> b (...)"), (coarse_token_ids, semantic_token_ids)
        )

        offsets = self.codebook_size * arange(self.num_coarse_quantizers)
        offsets = repeat(
            offsets, "q -> 1 (n q)", n=ceil_div(coarse_token_ids.shape[-1], self.num_coarse_quantizers)
        )
        ...
        coarse_quantize_tokens = repeat(
            self.coarse_quantize_embedding.weight,
            "q d -> (n q) d",
        ...
        tokens = torch.cat(
            (semantic_start_tokens, semantic_tokens, coarse_start_tokens, coarse_tokens), dim=1
        )
        ...
        if exists(self.transformer.rel_pos_bias):
            ...
            attn_bias = torch.where(is_cross_attn, self.cross_attn_bias, attn_bias)
        ...
        pred_semantic_tokens, pred_coarse_tokens = (
            tokens[:, :semantic_seq_len],
            tokens[:, (semantic_seq_len + 1) :],
        )

        # semantic logits

        semantic_logits = (
            self.to_semantic_logits(pred_semantic_tokens)
            if not return_only_coarse_logits and exists(self.to_semantic_logits)
            else None
        )
        ...
        pred_coarse_tokens_groupable, pred_coarse_tokens_remainder = (
            pred_coarse_tokens[:, :nq],
            pred_coarse_tokens[:, nq:],
        )

        pred_coarse_tokens_groupable = rearrange(
            pred_coarse_tokens_groupable, "b (n q) d -> b n q d", q=self.num_coarse_quantizers
        )

        coarse_logits_groupable = einsum(
            "q c d, b n q d -> b n q c", self.coarse_logit_weights, pred_coarse_tokens_groupable
        )
        ...
        if remainder_num_quantizers > 0:
            coarse_logits_remainder = einsum(
                "q c d, b q d -> b q c",
...
class FineTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        cond_drop_prob: float = 0.5,
        grad_shrink_alpha: float = 0.1,
        project_coarse_logits: bool = True,
        pad_id: int = -1,
        rel_pos_bias: bool = True,
        flash_attn: bool = False,
        **kwargs: ty.Any,
        ...
        self.coarse_logit_weights = (
            nn.Parameter(torch.randn(num_coarse_quantizers, codebook_size, dim))
            if project_coarse_logits
            else None
        )
    ...
    def forward(
        ...
        coarse_token_ids, fine_token_ids = map(
            lambda t: rearrange(t, "b ... -> b (...)"), (coarse_token_ids, fine_token_ids)
        )
        ...
        fine_quantize_tokens = repeat(
            self.fine_quantize_embedding.weight,
            "q d -> (n q) d",
        ...
        if exists(self.pos_bias_mlp):
            ...
            start_token_mask = rearrange(is_start_token_seq, "i -> i 1") | rearrange(
                is_start_token_seq, "j -> 1 j"
            )
        ...
        pred_coarse_tokens = rearrange(
            pred_coarse_tokens, "b (n q) d -> b n q d", q=self.num_coarse_quantizers
        )
        ...
        pred_fine_tokens_groupable, pred_fine_tokens_remainder = (
            pred_fine_tokens[:, :nq],
            pred_fine_tokens[:, nq:],
        )

        pred_fine_tokens_groupable = rearrange(
            pred_fine_tokens_groupable, "b (n q) d -> b n q d", q=self.num_fine_quantizers
        )

        fine_logits_groupable = einsum(
            "q c d, b n q d -> b n q c", self.fine_logit_weights, pred_fine_tokens_groupable
        )
        ...
        if remainder_num_quantizers > 0:
            fine_logits_remainder = einsum(
                "q c d, b q d -> b q c",
                self.fine_logit_weights[:remainder_num_quantizers],
...
class SemanticTransformerWrapper(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        audio_conditioner: Optional[nn.Module] = None,
        pad_id=-1,
        ...
        Args:
            transformer (SemanticTransformer): _description_
            wav2vec (Optional[Union[FairseqVQWav2Vec, HubertWithKmeans]], optional): _description_. Defaults to None.
            audio_conditioner (Optional[AudioConditionerBase], optional): _description_. Defaults to None.
            pad_id (int, optional): _description_. Defaults to -1.
            unique_consecutive (bool, optional): _description_. Defaults to True.
            mask_prob (float, optional): _description_. Defaults to 0.15.
        """
        super().__init__()
        ...
        assert not (
            exists(audio_conditioner) and not transformer.has_condition
        ), "if conditioning on audio embeddings from mulan, transformer has_condition must be set to True"

        assert (
            not exists(self.wav2vec) or self.wav2vec.codebook_size == transformer.num_semantic_tokens
        ), f"num_semantic_tokens on SemanticTransformer must be set to {self.wav2vec.codebook_size}"
    ...
    def generate(
        ...
        **kwargs,
        ...
        if exists(prime_wave):
            ...
            ids = self.wav2vec(prime_wave, flatten=False, input_sample_hz=prime_wave_input_sample_hz)
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert exists(raw_wave) or exists(
            semantic_token_ids
        ), "either raw waveform (raw_wave) is given or semantic token ids are given (semantic_token_ids)"
        ...
        loss = F.cross_entropy(
            rearrange(logits, "b n c -> b c n"), semantic_token_ids, ignore_index=self.pad_id
        )
...
class CoarseTransformerWrapper(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        pad_id=-1,
        unique_consecutive=True,
        semantic_cross_entropy_loss_weight=1.0,
        mask_prob=0.15,
    ):
        super().__init__()
        self.codec = codec
    ...
    def generate(
        ...
        **kwargs,
        ...
        assert not (
            exists(prime_wave) and exists(prime_coarse_token_ids)
        ...
        if exists(prime_coarse_token_ids):
            ...
            with torch.inference_mode():
                ...
                _, indices, _ = self.codec(
                    prime_wave, return_encoded=True, input_sample_hz=prime_wave_input_sample_hz
                )
        ...
        for time_step in tqdm(range(init_coarse_time_step, max_time_steps), desc="generating coarse"):
            for ind in range(self.num_coarse_quantizers):
                ...
                (_, coarse_logits), (
                    next_kv_cache,
                    next_embed_cache,
                ) = self.transformer.forward_with_cond_scale(
    ...
    def forward(
        ...
        **kwargs,
        ...
        if exists(self.audio_conditioner):
            assert exists(raw_wave)
            assert not exists(text) and not exists(text_embeds)
            text_embeds = self.audio_conditioner(
                wavs=raw_wave, namespace="coarse"
            )  # technically audio embeds, but shared text-audio joint embedding space for mulan
        ...
        if not exists(coarse_token_ids):
            ...
            with torch.inference_mode():
                ...
                assert (
                    indices.shape[0] == batch and indices.shape[1] == num_frames
        ...
        coarse_logits, semantic_logits = map(
            lambda t: maybe(rearrange)(t, "b n c -> b c n"), (coarse_logits, semantic_logits)
        )

        if self.unique_consecutive:
            num_coarse_logits, _num_semantic_logits = (
                coarse_labels.numel(),
        ...
        if self.semantic_cross_entropy_loss_weight > 0 and exists(semantic_logits):
            ...
            semantic_loss = F.cross_entropy(semantic_logits, semantic_labels, ignore_index=self.pad_id)

        coarse_loss = F.cross_entropy(coarse_logits, coarse_labels, ignore_index=self.pad_id)
...
class FineTransformerWrapper(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        codec: Optional[Union[SoundStream, EncodecWrapper]] = None,
        audio_conditioner: Optional[nn.Module] = None,
        coarse_cross_entropy_loss_weight=1.0,
        pad_id=-1,
        mask_prob=0.15,
    ):
        super().__init__()
        ...
        if exists(codec):
            assert (self.num_fine_quantizers + self.num_coarse_quantizers) == (
                codec.num_quantizers * codec.rq_groups
            ), "number of fine and coarse quantizers on fine transformer must add up to total number of quantizers on codec"
    ...
    def generate(
        ...
        **kwargs,
        ...
        assert not (
            exists(prime_wave) and exists(prime_fine_token_ids)
        ), "you can either pass in the prime as a raw wave (codec required) or as preprocessed acoustic token ids"
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert exists(raw_wave) ^ (
            exists(token_ids) ^ (exists(coarse_token_ids) and exists(fine_token_ids))
        ), "either raw waveform (raw_wav) is given, or coarse and fine token ids (coarse_token_ids, fine_token_ids)"
        ...
        if exists(token_ids):
            coarse_token_ids, fine_token_ids = (
                token_ids[..., : self.num_coarse_quantizers],
                token_ids[..., self.num_coarse_quantizers :],
            )
        ...
        coarse_logits, fine_logits = map(
            lambda t: maybe(rearrange)(t, "b n c -> b c n"), (coarse_logits, fine_logits)
        )
        ...
        fine_loss = F.cross_entropy(fine_logits, fine_labels, ignore_index=self.pad_id)
...
class AudioLM(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        assert (
            fine_transformer.num_coarse_quantizers + fine_transformer.num_fine_quantizers
        ) == codec.num_quantizers, f"fine_transformer.num_coarse_quantizers={fine_transformer.num_coarse_quantizers} + fine_transformer.num_fine_quantizers={fine_transformer.num_fine_quantizers} != codec.num_quantizers={codec.num_quantizers}"
        ...
        self.fine = FineTransformerWrapper(
            codec=codec, transformer=fine_transformer, audio_conditioner=audio_conditioner
        )

...
def check_one_trainer() -> None:
    """Checks that only one trainer exists."""
...
DATASET_FIELD_TYPE_CONFIG = dict(
    raw_wave=Annotated[torch.Tensor, Is[lambda t: t.dtype == torch.float and t.ndim in {2, 3}]],
    text=List[str],
    text_embeds=Annotated[torch.Tensor, Is[lambda t: t.dtype == torch.float and t.ndim == 3]],
)
...
def yes_or_no(question: str) -> bool:
    """Asks a question."""
    answer = "yes"  # input(f"{question} (y/n) ")
...
def determine_types(data: ty.Sequence[ty.Any], config: dict) -> ty.Tuple[ty.Any, ...]:
    """
    Args:
        data (ty.Sequence[ty.Any]): _description_
        config (dict): _description_
    Raises:
        TypeError: _description_
    Returns:
        ty.Tuple[ty.Any]: _description_
    """
    output = []
    for el in data:
...
class SoundStreamTrainer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if accelerator:
            ...
            self.accelerator = Accelerator(
                kwargs_handlers=[kwargs], split_batches=split_batches, **accelerate_kwargs
            )
        ...
        self.discr_optim = get_optimizer(
            soundstream.stft_discriminator.parameters(), lr=lr, wd=wd, use_lion=use_lion
        )
        ...
        (self.soundstream, self.optim, self.discr_optim, self.dl) = self.accelerator.prepare(
            self.soundstream, self.optim, self.discr_optim, self.dl
        )
        ...
        ), "FSDP not supported for soundstream trainer due to complex-valued stft discriminator"
    ...
    def is_distributed(self):
        return not (
            self.accelerator.distributed_type == DistributedType.NO and self.accelerator.num_processes == 1
        )
    ...
    def train_step(self) -> dict:
        ...
        for _ in range(self.grad_accum_every):
            ...
            loss, (
                ...
                feature_loss,
                all_commitment_loss,
            ) = self.soundstream(wave, return_loss_breakdown=True)
        ...
        if exists(self.discr_max_grad_norm):
            self.accelerator.clip_grad_norm_(
                self.soundstream.stft_discriminator.parameters(), self.discr_max_grad_norm
            )
        ...
        if self.is_main and not (steps % self.save_results_every):
            ...
            for model, label in models:
                ...
                for ind, recon in enumerate(recons.unbind(dim=0)):
                    ...
                    torchaudio.save(
                        filename, recon.cpu().detach(), self.unwrapped_soundstream.target_sample_hz
                    )
    ...
    def train(  # type: ignore # pylint: disable=arguments-renamed
        self,
        log_fn: ty.Callable = noop,
    ) -> None:
        """Training entrypoint."""
...
class SemanticTransformerTrainer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.accelerator = Accelerator(split_batches=split_batches, **accelerate_kwargs)
        ...
        (self.train_wrapper, self.optim, self.dl, self.valid_dl) = self.accelerator.prepare(
            self.train_wrapper, self.optim, self.dl, self.valid_dl
        )
    ...
    def data_tuple_to_kwargs(self, data):
        ...
        x = dict(zip(self.ds_fields, data))
        logger.debug(x)
        return x
    ...
    def train(self, log_fn: ty.Callable = noop) -> None:
        """Train."""
        while self.steps < self.num_train_steps:
            logs = self.train_step()
            log_fn(logs)
    ...
    def __init__(
        ...
        *,
        ...
        self.train_wrapper = CoarseTransformerWrapper(
            codec=codec, wav2vec=wav2vec, transformer=transformer, audio_conditioner=audio_conditioner
        )
        ...
        (self.transformer, self.optim, self.dl, self.valid_dl) = self.accelerator.prepare(
            self.transformer, self.optim, self.dl, self.valid_dl
        )
    ...
    def __init__(
        ...
        *,
        ...
        self.train_wrapper = FineTransformerWrapper(
            codec=codec, transformer=transformer, audio_conditioner=audio_conditioner
        )

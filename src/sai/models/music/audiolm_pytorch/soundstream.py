    ...
    def __init__(self, channels=16, layers=4, groups=(4, 16, 64, 256), chan_max=1024, input_channels=1):
        ...
        for _, group in zip(range(layers), groups):
            ...
            self.conv_layers.append(
                nn.Sequential(
                    nn.Conv1d(curr_channels, chan_out, 41, stride=4, padding=20, groups=group), leaky_relu()
                )
            )
...
class SqueezeExcite(nn.Module):
    def __init__(self, dim, reduction_factor=4, dim_minimum=8):
        ...
        self.net = nn.Sequential(
            nn.Conv1d(dim, dim_inner, 1), nn.SiLU(), nn.Conv1d(dim_inner, dim, 1), nn.Sigmoid()
        )
...
class Residual(nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            fn (nn.Module): Module to add the residual function to.
        """
        super().__init__()
        self.fn = fn
        self.pad_mode = pad_mode

    def forward(self, x: torch.Tensor, **kwargs: ty.Any) -> torch.Tensor:
        """Adds X back to `fn(X)` to achieve a residual unit."""
        # Run through layer
        out: torch.Tensor = self.fn(x, **kwargs)
        logger.trace(f"out={out.size()}; x={x.size()}")
        # Need to pad if dim mismatch
        if out.size(-1) < x.size(-1):
            n = int(x.size(-1) - out.size(-1))
            p1d = (int(n / 2), n - int(n / 2))  # pad last dim by n/2 on each side
            out = F.pad(out, pad=p1d, mode=self.pad_mode)  # pylint: disable=not-callable
        # Residual layer
        x = out + x
        return x
...
class CausalConv1d(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: ty.Any,
        ...
        """
        Args:
            chan_in (int): _description_
            chan_out (int): _description_
            kernel_size (int): _description_
            pad_mode (str, optional): _description_. Defaults to "reflect".
        """
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        logger.trace(f"x={x.size()}; causal_padding={self.causal_padding}")
        p = self.causal_padding if self.causal_padding < int(x.size(-1)) else int(x.size(-1) - 1)
        x = F.pad(x, (p, 0), mode=self.pad_mode)  # pylint: disable=not-callable
...
def EncoderBlock(
    ...
    chan_out: int,
    stride: int,
    cycle_dilations: ty.Tuple[int, int, int] = (1, 3, 9),
    squeeze_excite: bool = False,
    pad_mode: str = "reflect",
) -> nn.Sequential:
    """_summary_
    Args:
        ...
        cycle_dilations (ty.Tuple[int, int, int], optional): _description_. Defaults to (1, 3, 9).
        squeeze_excite (bool, optional): _description_. Defaults to False.
        pad_mode (str, optional): _description_. Defaults to "reflect".
    Returns:
    ...
    return nn.Sequential(
        ...
        residual_unit(chan_in, chan_in, next(it)),
        residual_unit(chan_in, chan_in, next(it)),
        CausalConv1d(chan_in, chan_out, kernel_size=2 * stride, stride=stride),
    )


def DecoderBlock(
    chan_in, chan_out, stride, cycle_dilations=(1, 3, 9), squeeze_excite=False, pad_mode="reflect"
):
...
class SoundStream(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.discriminators = nn.ModuleList(
            [MultiScaleDiscriminator() for _ in range(len(discr_multi_scales))]
        )
        discr_rel_factors = [int(s1 / s2) for s1, s2 in zip(discr_multi_scales[:-1], discr_multi_scales[1:])]
        self.downsamples = nn.ModuleList(
            [nn.Identity()]
            + [nn.AvgPool1d(2 * factor, stride=factor, padding=factor) for factor in discr_rel_factors]
        )
    ...
    def save(self, path):
        path = Path(path)
        pkg = dict(model=self.state_dict(), config=self._configs, version=__version__)
    ...
    def load(self, path, strict=True):
        ...
        if "version" in pkg and version.parse(pkg["version"]) < parsed_version:
            print(
                f'soundstream model being loaded was trained on an older version of audiolm-pytorch ({pkg["version"]})'
            )
    ...
    def forward(
        ...
        if exists(is_denoising):
            denoise_input = torch.tensor(
                [is_denoising, not is_denoising], dtype=x.dtype, device=self.device
        ...
        if return_discr_loss:
            ...
            if self.single_channel:
                ...
                stft_real_logits, stft_fake_logits = map(
                    self.stft_discriminator, (real.requires_grad_(), fake)
                )
        ...
        for real_intermediates, fake_intermediates in discr_intermediates:
            losses = [
                F.l1_loss(real_intermediate, fake_intermediate)
...
def AudioLMSoundStream(
    strides: ty.Tuple[int, int, int, int] = (2, 4, 5, 8),
    target_sample_hz: int = 16000,
    rq_num_quantizers: int = 12,
    **kwargs: ty.Any,
):
    """Initializer for SoundStream."""
    return SoundStream(
        strides=strides,
...
def MusicLMSoundStream(
    strides: ty.Tuple[int, int, int, int] = (3, 4, 5, 8),
    target_sample_hz: int = 24000,
    rq_num_quantizers: int = 12,
    **kwargs: ty.Any,
):
    """Initializer for SoundStream."""

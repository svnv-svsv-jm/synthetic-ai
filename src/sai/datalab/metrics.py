...
from .synthesizers.utils import check_common_data_format
from .target_data_manipulation import _generate_column_type_dictionary

from .cython.cython_metric import mixed_distance


def _bin_data(target_col, syn_col, col_type: str, number_of_bins: int):  # type: ignore
    """
    ...
    if col_type == "number":
        ...
        if cardinality > number_of_bins:
            top_cat = (
                target_col.value_counts().sort_values(ascending=False)[: number_of_bins - 1].index.to_list()
            )
            other = (
                target_col.value_counts().sort_values(ascending=False)[number_of_bins - 1 :].index.to_list()
            )
        ...
        raise ValueError("Col type not recognized")
    return binned_target, binned_syn


def _bin_looped(target, synthetic, column_dictionary: dict, number_of_bins: int):  # type: ignore
    t = pd.DataFrame()
    s = pd.DataFrame()
...
def _prepare_data_for_privacy_metrics(
    ...
    for column_name, column_type in column_dictionary.items():
        if column_type == "category":
            ...
            raise NotImplementedError(f"{column_type} Type not supported")

    # drop id col since it's not needed
    tgt_data_p = tgt_data_p.reset_index().drop("id", axis=1)
    syn_data_p = syn_data_p.reset_index().drop("id", axis=1)
...
def _get_nn_model(train: DataFrame, cat_slice: int) -> Tuple[np.ndarray]:
    ...
    return nearest_neighbor_model  # type: ignore
...
def _calculate_dcr_nndr(
    ...
    tgt_query_neighbors = nn_model.kneighbors(tgt_query, n_neighbors=2)  # type: ignore
    syn_query_neightbors = nn_model.kneighbors(syn_query, n_neighbors=2)  # type: ignore
    ...
    def _bootstrap_quantile(
        ...
        return _bootstrap_func(
            series,
            lambda series_: np.quantile(series_, quantile),  # type: ignore
            bootstrap_kwargs,
        )
    ...
    def _quantile_test_function(target: Series, synthetic: Series) -> Dict:
        ...
        * we fail the test if any of the synthetic quantiles is below the
        ...
        for quantile in quantiles:
            bootstrap_results.loc[quantile, ["tgt", "tgt_ci_low", "tgt_ci_high"]] = _bootstrap_quantile(
                target, quantile, bootstrap_kwargs
            )
            bootstrap_results.loc[quantile, "syn"] = np.quantile(synthetic, quantile)

        bootstrap_results["check"] = bootstrap_results["syn"] >= bootstrap_results["tgt_ci_low"]
...
def _calculate_coherence(  # type: ignore
    tgt_data: DataFrame,
    syn_data: DataFrame,
    ...
    tgt_binned, syn_binned = _bin_looped(tgt_data, syn_data, column_dict, number_of_bins)  # type: ignore
...
def _calculate_statistical_distances(  # type: ignore
    tgt_data: DataFrame,
    syn_data: DataFrame,
    type: str,  # pylint: disable=redefined-builtin
    number_of_bins: int = 10,
    ...
    def _sample_one_event(data: DataFrame) -> DataFrame:
        ...
        draws = pd.DataFrame(
            {"sequence_pos": np.floor(np.random.rand(len(seq_lens)) * seq_lens).astype(int)}
        ).reset_index()
    ...
    tgt_binned, syn_binned = _bin_looped(tgt_sample, syn_sample, column_dict, number_of_bins)
    tgt_binned = tgt_binned.astype(str)
    syn_binned = syn_binned.astype(str)

    def calc_shares(df_binned, label):  # type: ignore
    ...
    for i in range(cross.shape[0]):
        ...
        tgt_values = (
            tgt_binned[col_1] + "|" + tgt_binned[col_2] + "|" + tgt_binned[col_3] + "|" + tgt_binned[col_4]
        )
        syn_values = (
            syn_binned[col_1] + "|" + syn_binned[col_2] + "|" + syn_binned[col_3] + "|" + syn_binned[col_4]
        )
        shares = pd.merge(
            calc_shares(tgt_values, "tgt"),  # type: ignore
            calc_shares(syn_values, "syn"),  # type: ignore
            how="left",
        )
        shares["syn"] = shares["syn"].fillna(0)
        diff = np.abs(shares["tgt"] - shares["syn"])
...
def _calculate_privacy_tests(tgt_data: DataFrame, syn_data: DataFrame) -> ty.Tuple[dict, dict]:
    """
...
def compare(
    tgt_data: DataFrame,
    syn_data: DataFrame,
    metrics_to_return: List[str] = ["statistical-distances", "privacy-tests"],
) -> Dict:
    """Compare a target and synthetic dataset and returns metrics.
    Args:
        tgt_data (DataFrame): target data
        syn_data (DataFrame): synthetic data
        metrics_to_return (List, optional): list of metric types to return, if empty compute all
    ['statistical-distances', 'privacy-tests']
    Returns:
        Dict: Metrics dataframe.
    """
    # Init
    ...
    if "statistical-distances" in metrics_to_return:
        ...
        stat_upc = _calculate_coherence(tgt_data, syn_data, "users_per_category")  # type: ignore
        stat_cpu = _calculate_coherence(tgt_data, syn_data, "categories_per_user")  # type: ignore
        metrics_dict["MAX univariate"] = _agg(stat_1dim["max"])  # type: ignore
        metrics_dict["L1D univariate"] = _agg(stat_1dim["l1d"])  # type: ignore
        metrics_dict["L1D bivariate"] = _agg(stat_2dim["l1d"])  # type: ignore
        metrics_dict["L1D 3-way"] = _agg(stat_3dim["l1d"])  # type: ignore
        metrics_dict["L1D 4-way"] = _agg(stat_4dim["l1d"])  # type: ignore
        metrics_dict["L1D Users per Category"] = _agg(stat_upc["l1d"])  # type: ignore
        metrics_dict["L1D Categories per User"] = _agg(stat_cpu["l1d"])  # type: ignore
    # Privacy metrics
    if "privacy-tests" in metrics_to_return:
        checks, privacy_checks = _calculate_privacy_tests(tgt_data, syn_data)
        metrics_dict["DCR test"] = checks["DCR"]
        metrics_dict["NNDR test"] = checks["NNDR"]
        metrics_dict["privacy checks"] = privacy_checks  # type: ignore
    # Return
    return metrics_dict


def _agg(x: ty.Union[int, float]) -> float:
    x = np.round(np.mean(x), 5)
    return x

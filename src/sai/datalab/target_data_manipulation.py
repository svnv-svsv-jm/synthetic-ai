...
__all__ = ["prepare_common_data_format", "encode_simple_common_data_format", "split_train_val"]
...
def prepare_common_data_format(
    ...
    assert "id" in columns, f"'id' column not in dataframe: {columns}. Please rename if possible."
            ...
            if col in cat_columns_convert:
                ...
                if df.loc[:, col].isnull().sum() != 0:
                    try:
                        ...
                        logger.debug(exc)
                        df.loc[:, col] = df.loc[:, col].astype("category").cat.add_categories("").fillna("")
...
def encode_simple_common_data_format(
    ...
    Args:
        ...
        shared_encoding_cols (List[str], optional): list of columns that share the same encoding. Defaults to None.
        encoding (str, optional): encoding method (default label). Defaults to "label".
    Returns:
        DataFrame: encoded pandas dataframe
    """
    if encoding == "label":
        ...
        if shared_encoding_cols is not None:
            ...
            categories_not_shared_encoding = [
                cat_col for cat_col in category_columns if cat_col not in shared_encoding_cols
            ]
...
def _generate_column_type_dictionary(data: pd.DataFrame) -> dict:

# pylint: disable=unsupported-assignment-operation,unused-argument

import pandas as pd
import numpy as np
from .base import BaseSynthesizer
from ..target_data_manipulation import _generate_column_type_dictionary
from .utils import _assign_column_type
...
class ShuffleSynthesizer(BaseSynthesizer):
    ...
    def train(self, target_data: pd.DataFrame, *args):  # type: ignore
        super().train(target_data)
        self.target_data_ = pd.DataFrame(target_data)

    def generate(self, number_of_subjects):  # type: ignore
        super().generate(number_of_subjects)

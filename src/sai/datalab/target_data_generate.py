...
# __all__ = ["generate_simple_seq_dummy"]
...
# def generate_simple_seq_dummy(
#     number_of_subjects: int,
#     sequence_length: list,
#     num_numeric_col: int,
#     num_categorical_col: int,
# ) -> pd.DataFrame:
#     """
...
#         else:
#             sequence_possible_lengths = np.arange(  # type: ignore
#                 start=sequence_length[0], stop=sequence_length[1], step=1
#             )

#         random_sequence_lengths = [
#             np.random.choice(sequence_possible_lengths) for i in range(number_of_subjects)
#         ]
#         sequence_index = [
#             sequence_exploded
#             for sequence_length in random_sequence_lengths
#             for sequence_exploded in range(sequence_length)
#         ]
#         id_index = [
#             subject_id_list
#             for subject_id, sequence_length in zip(
#                 range(number_of_subjects), random_sequence_lengths
#             )
#             for subject_id_list in [subject_id] * sequence_length
#         ]
#         tuples = list(zip(id_index, sequence_index))
#         df_dummy = pd.DataFrame(
#             index=pd.MultiIndex.from_tuples(tuples, names=["id", "sequence_pos"])
#         )
